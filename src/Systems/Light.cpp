...
void is::systems::LightSystem::awake()
{
    for (auto &elem : _componentManager->getComponentsByType(typeid(is::components::LightComponent).hash_code())) {
        ...
        if (!ptr)
            throw new is::exceptions::Exception("LightSystem", "Could not get LightComponent pointer");
        ...
        for (auto &wc : _componentManager->getComponentsByType(typeid(is::components::WindowComponent).hash_code())) {
            ...
            if (!ptr_window)
                throw new is::exceptions::Exception("LightSystem", "Could not get WindowComponent pointer");
        ...
        if (!windowFound)
            throw new is::exceptions::Exception("LightSystem", "Could not found window");
